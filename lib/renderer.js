...
class Renderer {
  static output(person) {
    const { name, gender, age, ageGroup, type, alignment, race, klass, background, specialty, personalityTraits, ideal, bond, flaw, mannerism, talent, trait, characteristic, DNA } = person;
    const appearance = DNA.traits;

    const renderAppearance = (legend) => {
      if (appearance[legend] === undefined) return ''; // make sure this race has the legend
      if ((legend === 'skin-aging') && (ageGroup !== 'old')) return ''; // only apply aging if we're in the old ageGroup
