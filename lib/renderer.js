...
class Renderer {
  ...
  static toConsole(person) {
    ...
    const consoleOutput = `Introduction \n${intro}\n\n` +
                          `Abilities \n${abilities}`;

    process.stdout.write(colors.white(consoleOutput));
  }

  static toMarkdown(person) {
    const intro = Renderer.renderIntro(person);
    let abilities = Renderer.renderAbilities(person);
    // Replace \t characters with a hyphen for Markdown unordered lists
    abilities = abilities.replace(/\t/g, '-');
    const markdownOutput = `## Introduction \n${intro}\n\n` +
                           `## Abilities \n${abilities}`;
