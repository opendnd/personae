...
class Renderer {
  ...
  static renderIntro(person) {
    const { name, age, ageGroup, alignment, klass, background, specialty, personalityTraits, ideal, bond, mannerism, talent, trait, DNA } = person;
    let flaw = person;
    if (flaw[0] != 'I') {
      flaw = flaw.replace(flaw[0], flaw[0].toLowerCase());
    }
    const { gender, race } = DNA;
