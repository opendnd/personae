...
const geneticaDir = path.join(modulesDir, 'genetica');
const geneticaLoader = require(path.join(geneticaDir, 'lib', 'loader'));
...
const wizardDNA = (outputDir, DNApath) => {
  ...
  // load DNA
  const DNA = geneticaLoader(DNApath);
  ...
  questions.askMany({
    ...
    questions.askOne({ info: colors.cyan('Would you like to save your person? (y | n)') }, (result) => {
