...
class Personae {
  ...
  // generate apperance
  generateAppearance(gender = 'female', ageGroup = 'young') {
    const characteristic = defaults.appearance.characteristic.sample();
    const eyes = {
      shape: defaults.appearance.eyes.shape.sample(),
      color: defaults.appearance.eyes.color.sample(),
      brows: defaults.appearance.eyes.brows.sample(),
    };
    const skin = {
      color: defaults.appearance.skin.color.sample(),
      general: defaults.appearance.skin.general.concat(defaults.appearance.skin[ageGroup]).sample(),
    };
    const face = {
      shape: defaults.appearance.face.shape.sample(),
      nose: defaults.appearance.face.nose.sample(),
      mouth: defaults.appearance.face.mouth.sample(),
    };
    const hair = {
      general: defaults.appearance.hair.general.concat(defaults.appearance.hair[gender]).sample(),
      color: defaults.appearance.hair.color.sample(),
      facial: (gender === 'male') ? defaults.appearance.hair.facial.sample() : undefined,
    };
    const body = {
      general: defaults.appearance.body.general.concat(defaults.appearance.body[gender]).sample(),
    };
  ...
  generate(opts = {}) {
    ...
    const appearance = this.generateAppearance(gender, ageGroup);
