...
class Personae {
  ...
  validateOpts(opts = {}) {
    ...
    // name
    if (opts.name === undefined) opts.name = Nomina.generate({ type: opts.gender, theme: opts.theme });
    if (opts.name === '') opts.name = Nomina.generate({ type: opts.gender, theme: opts.theme });
  ...
  // generate abilities
  generateAbilities(race = 'Dragonborn') {
  ...
  generateParents(person) {
    const { DNA, type, theme, race } = person;
    const genetica = new Genetica();
  ...
  generate(opts = {}) {
    ...
    const { type, race, klass, alignment, theme, name, gender, age, background } = genOpts;

    // generate person details
    const ageGroup = this.generateAgeGroup(race, age);
    const abilities = this.generateAbilities(race);
